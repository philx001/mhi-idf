...
| Rôle | Description |
|------|-------------|
| **Responsable siège** | Administrateur réseau (siège). Peut inviter, gérer les rôles, voir toutes les églises et les modifier. |
...
| Fonctionnalité | Responsable siège | Responsable église (hors Croissy) | Responsable église (Croissy) | Membre |
|----------------|-------------------|------------------------------------|-------------------------------|--------------|
...
| Attribuer / modifier rôle, retirer d’église, révoquer / restaurer accès | ✅ Tous les utilisateurs | ✅ Uniquement membres de son église | ✅ Tous (sauf modifier un responsable siège) | ❌ Non |
...
| **Planning partagé** – Créer des sessions | ✅ Oui (siège uniquement en BDD) | ❌ Non | ❌ Non | ❌ Non |
...
- **Qui y a accès** :
  ...
  - Le **responsable siège** peut voir (et modifier) le calendrier de n’importe quelle église.
...
- **Rôles** : type `AppRole` dans `app/admin/actions.ts` (`responsable_siège` \| `responsable_eglise` \| `membre`).
